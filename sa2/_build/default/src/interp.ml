...
let lookup_class : string -> class_env -> class_decl ea_result  = fun c_name c_env ->
  ...
  | Some (super, fields, methods) -> return (super, fields, methods)
    
let rec new_env : string list -> env ea_result  = fun fs ->
  match fs with 
  | [] -> lookup_env
  | id::ids -> (extend_env id (RefVal (Store.new_ref g_store (NumVal 0)))) >>+
  new_env ids
...
let lookup_method : string -> string -> class_env ->
  ...
  | None -> None
  | Some (_, _, methods) -> 
    (match List.assoc_opt m_name methods with 
    | None -> None
    | Some m -> Some m)
