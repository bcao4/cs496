...
let rec level t i = 
  match i, t with 
  | 1, Node(d,lt,rt) -> [d]
  | n, Node(d,lt,rt) -> level lt (i-1) @ level rt (i-1)
  | n, Empty -> []
...
let rec height t = 
  ...
  | Node(a, Empty, Empty) -> 1
  | Node(a, lt, Empty) -> 1 + height lt
  | Node(a, Empty, rt) -> 1 + height rt
  | Node(a, lt, rt) -> 1 + max (height lt) (height rt) *)
